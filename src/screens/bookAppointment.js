...
import { View, Text, StyleSheet, TouchableOpacity, Alert, TextInput, ScrollView, Button, Platform } from 'react-native'
...
export default function BookAppointment({ navigation, route }) {
    ...
    async function openImagePicker() {
        try {
            ...
            setResult([pickerResult])
            console.log('pickerResult', pickerResult.assets[0]);
            RNFetchBlob.fs
                .readFile( pickerResult.assets[0]?.uri, 'base64')
                .then((data) => {
                    setBaseData(data)
                })
                .catch((err) => {
                    console.log('Image Picker Base64 Data Error', err);
                });
        } catch (e) {
            ...
            <View style={[styles.box, { width: ' 85%', flexDirection: 'column', alignItems: 'flex-start' }]}>
                ...
                <View style={{ marginTop: 10, width: '95%', alignSelf: 'center' }}>
                    ...
                    <Button
                        ...
                        onPress={() => {
                            Platform.OS === 'android' ? uploadDocument() : openImagePicker()
                        }}
                    />
                </View>
            </View>
